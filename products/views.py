...
from .serializers import CategorySerializer, ImageSerializer, BookSerializer, \
    CartItemSerializer, BookSerializer1, CustomBookSerializer, CategorySerializer1
...
class BookViewSet(viewsets.ModelViewSet):
    ...
    # def imageList(self, request, pk=None):
    #     book = self.get_object()
    #     images = Image.objects.filter(id_book=book)
    #     serializer = ImageSerializer(images, many=True)
    #     return Response(serializer.data)

    # @action(detail=True, methods=['get'])
    # def categoryList(self, request, pk=None):
    #     book = self.get_object()
    ...
    def list(self, request, *args, **kwargs):
        # Optimize for list view if needed
    ...
    def all_books(self, request):
        # Retrieve all books without pagination
...
class ImageViewSet(viewsets.ModelViewSet):
    ...
    def images_by_book(self, request, book_id=None):
        ...
        return Response(serializer.data)
    # @action(detail=False, methods=['get'], url_path='book-with-images/(?P<book_id>\d+)')
    # def book_with_images(self, request, book_id=None):
    #     book = Book.objects.prefetch_related(
    #         Prefetch('images', queryset=Image.objects.only('id', 'image_url'))
...
class CartItemViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['post'], url_path='add-to-cart/(?P<id_user>\d+)', permission_classes=[IsAuthenticated])
    def add_to_cart(self, request, id_user):
        print(f"Authenticated User ID: {request.user.id}")
        print(f"Requested User ID: {id_user}")
        try:
            data = json.loads(request.body)
            user = get_object_or_404(User, id=id_user)

            # if str(request.user.id) != id_user:
            #     return Response({"detail": "You don't have permission to modify this cart"}, status=status.HTTP_403_FORBIDDEN)
            ...
            if len(data) == 1:
                return Response({'idCart': added_items[0]['idCart']}, status=status.HTTP_201_CREATED)
    ...
    def update_cart_item(self, request, id_cart, id_user):
        try:
            ...
            # Find the cart item using id_cart
            cart_item = get_object_or_404(CartItem, id_cart=id_cart)

            # Check if the user has permission to modify this cart item
    ...
    def get_cart_by_user(self, request, id_user=None):
        if str(request.user.id) != id_user:
            return Response({"detail": "You do not have permission to view this cart."}, status=status.HTTP_403_FORBIDDEN)
...
class BookImagesView(APIView):
    ...
    def get(self, request, id_book):
        # Lọc các hình ảnh theo id_book được cung cấp
        images = Image.objects.filter(id_book=id_book)
